ConvenientView {
	...
	*listWindow{ | buffers |
		...
		var initpath;
		var cond = Condition(false);
		var win, header, dragSource;
		var winW = 240, winH = 240;
		var fontName = "Monospace", fontSize = 16, headerFontSize = 18;
		

		// strings
		var headerString = "available folder keys";
		// colors
		var backgroundColor = Color.fromHexString("#42F874");
		var headerbackgroundColor = Color.fromHexString("#B39DDB");
		var headerStringColor = Color.fromHexString("#311B92");
		var dragSourcebackgroundColor = Color.clear;
		var dragSourceStringColor = Color.fromHexString("#311B92");
		...
		if (listWindowOpen.not, {
			...
			list = buffers.keys.collect { | keys |
				keys;
			};

			listGrid = GridLayout.new;
			
			list.do{ | key, i |
				var columnSize = 3;
				var rowSize = 5;
				var row = (i*rowSize.reciprocal).floor;
				var column = i.wrap(1,columnSize);
				win.layout_(
					listGrid.add(
						DragSource(win)
						//.align_(\center)
						.background_(Color.clear)
						.stringColor_(Color.fromHexString("#FF20AB"))
						.object_("\\"++"%".format(key))
						.font_(Font(fontName, fontSize, bold: true))
						.mouseOverAction_({ | view, x, y |
							win.drawFunc = {
								var gradient = this.listGradient.mirror;
								var rotate = 0; //(this.gradient.size/2).floor;
								gradient = gradient.rotate(rotate.asInteger);
								// fill gradient
								Pen.addOval(win.view.bounds.insetBy(dropOvalSize));
						...
						column,
						\center
					).vSpacing_(2).hSpacing_(2)
				).front
			};
			//buffers.size
			listGrid.spacing_(20);
			listGrid.margins_([10, 30, 10, 30]);
